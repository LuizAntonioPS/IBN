...
public class IncStatistics extends Statistics {
	...
	public static int MDL = 2;
	public static int MIT = 3;
	public static int PRECISION = 4;
	public static int RECALL = 5;
	public static int F_MEASURE = 6;
	public static int BATCH_EXTRA_EDGES = 7;
	public static int BATCH_MISSED_EDGES = 8;
	public static int BATCH_REVERSED_EDGES = 9;
	public static int STANDARD_EXTRA_EDGES = 10;
	public static int STANDARD_MISSED_EDGES = 11;
	public static int STANDARD_REVERSED_EDGES = 12;
	...
	public static void PrintMetrics(List<Integer> metrics, int step, BayesNet bayesNet, Instances instances, int split_threshold, String dataSet_name) throws Exception {
		String values = step + "";
		if(metrics.contains(ACC)) { values += " " + IncStatistics.calculateAccurary(bayesNet, instances, split_threshold); }
		if(metrics.contains(LL)) { values += " " + IncStatistics.calculateLogLoss(bayesNet, instances, split_threshold); }
		if(metrics.contains(MDL)) { values += " " + IncStatistics.measureMDLScore(bayesNet); }
		if(metrics.contains(MIT)) { values += " " + IncStatistics.measureMITScore(bayesNet); }
		if(metrics.contains(PRECISION) || metrics.contains(IncStatistics.RECALL) || metrics.contains(IncStatistics.F_MEASURE)) {
			double[] precisionRecallFMeasure = BayesNetUtils.precisionRecallFMeasure(bayesNet, dataSet_name);
			if(metrics.contains(PRECISION)) { values += " " + precisionRecallFMeasure[0]; }
			if(metrics.contains(RECALL)) { values += " " + precisionRecallFMeasure[1]; }
			if(metrics.contains(F_MEASURE)) { values += " " + precisionRecallFMeasure[2]; }
		}
		if(metrics.contains(IncStatistics.BATCH_EXTRA_EDGES) || metrics.contains(IncStatistics.BATCH_MISSED_EDGES) || metrics.contains(IncStatistics.BATCH_REVERSED_EDGES)) {
			BayesNetUtils.configBIFFFile(bayesNet, "bayesnets/" + dataSet_name + "_empty.xml", instances);
			if(metrics.contains(IncStatistics.BATCH_EXTRA_EDGES)) { values += " " + bayesNet.measureExtraArcs(); }
			if(metrics.contains(IncStatistics.BATCH_MISSED_EDGES)) { values += " " + bayesNet.measureMissingArcs(); }
			if(metrics.contains(IncStatistics.BATCH_REVERSED_EDGES)) { values += " " + bayesNet.measureReversedArcs(); }
		}
		if(metrics.contains(IncStatistics.STANDARD_EXTRA_EDGES) || metrics.contains(IncStatistics.STANDARD_MISSED_EDGES) || metrics.contains(IncStatistics.STANDARD_REVERSED_EDGES)) {
			BayesNetUtils.configBIFFFile(bayesNet, "bayesnets/original/" + dataSet_name + ".xml", null);
			if(metrics.contains(IncStatistics.STANDARD_EXTRA_EDGES)) { values += " " + bayesNet.measureExtraArcs(); }
			if(metrics.contains(IncStatistics.STANDARD_MISSED_EDGES)) { values += " " + bayesNet.measureMissingArcs(); }
			if(metrics.contains(IncStatistics.STANDARD_REVERSED_EDGES)) { values += " " + bayesNet.measureReversedArcs(); }
		}
