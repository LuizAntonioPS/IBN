...
public class BayesNetUtils {
	...
	public static int EMPTY_MODE = 0;
	public static int PARTIAL_MODE = 1;
	public static int STEP_SIZE_100 = 100;
	public static int STEP_SIZE_200 = 200;
	public static int STEP_SIZE_400 = 400;
	public static int STEP_SIZE_500 = 500;
	public static int STEP_SIZE_1000 = 1000;
	public static int STEP_SIZE_2000 = 2000;
	public static int STEP_SIZE_4000 = 4000;
	public static int ST_ALGORITHM = 0;
	public static int IHCS_ALGORITHM = 1;

	public static List<Integer> iHeadNeighbors;
	public static List<Integer> iTailNeighbors;
	
	public static BayesNet getBayesNet(String dataset, int mode) {
		BayesNet bayesNet = null;
		try {
			String s_mode = mode == EMPTY_MODE ? "empty" : "partial";
			bayesNet = (BayesNet) new BIFReader().processFile(String.format("bayesnets/%s_%s.xml", dataset, s_mode));
			bayesNet.initCPTs();
			visualizeBayesNet(bayesNet, String.format("Initial %s Structure (%s)", capitalize(dataset), s_mode));
		} catch (Exception e) {
			System.err.println("network not found");
		}
		return bayesNet;
	}
...
//		nodeNames.add("Satisfação do Cliente");
	...
	/**
	 * @param bayesNet learned network
	 * @return
	 * @throws Exception
	 */
	public static double[] precisionRecallFMeasure(BayesNet bayesNet, String dataSet_name) throws Exception {
		BayesNetUtils.writeBIFFFile(BayesNetUtils.getBIFFVersion("bayesnets/original/" + dataSet_name + ".xml"));
		BayesNet oBayesNet = (weka.classifiers.bayes.BayesNet) new BIFReader().processFile("bayesnets/to_compare.xml"); //original network
		double nTP = 0;
		double nFN = 0;
		for (int iAttribute = 0; iAttribute < oBayesNet.getNrOfNodes(); iAttribute++) {
			for (int iParent = 0; iParent < oBayesNet.getParentSet(iAttribute).getNrOfParents(); iParent++) {
				...
				if (bayesNet.getParentSet(iAttribute).contains(nParent)) {
					nTP++;
				} else {
					nFN++;
				}
		...
		double nFP = 0;
		for (int iAttribute = 0; iAttribute < bayesNet.getNrOfNodes(); iAttribute++) {
	...
	public static void configBIFFFile(BayesNet bayesNet, String fileLocal, Instances instances) throws IOException, Exception {
		if(instances != null) { bayesNet.setBIFFile(BayesNetUtils.writeBIFFFile(BayesNetUtils.getBatchBIFFVersion(fileLocal, instances)));}
		else { bayesNet.setBIFFile(BayesNetUtils.writeBIFFFile(BayesNetUtils.getBIFFVersion(fileLocal))); }
