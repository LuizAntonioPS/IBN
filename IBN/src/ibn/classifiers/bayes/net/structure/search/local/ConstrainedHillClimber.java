...
public class ConstrainedHillClimber extends LocalScoreSearchAlgorithm {
		...
		for (int iAttributeTail = 0; iAttributeTail < nNrOfAtts; iAttributeTail++) {
			if (iAttributeTail != iAttributeHead) {
				if (!parentSet.contains(iAttributeTail)) {
					// AQUI ATUALIZA OU ADD NO CACHE O SCORE DA OPERAÃ‡Ä‚O DE ADIÃ‡Ä‚O PARA TODOS OS
					// OUTROS NÃ“S QUE NÄ‚O SÄ‚O PAIS DA VARIÃ�VEL NA REDE ATUAL
					...
					// SE A VARIAVÃ‰L FO FOR FOR PAI DE VARIÃ�VEL JÃ� NA REDE BASE, ATUALIZA OU ADD NO
					// CACHE O SCORE DA OPERAÃ‡Ä‚O DE REMOÃ‡Ä‚O
	...
	/** Bayes net contendo todos os pais possÃ­veis **/
